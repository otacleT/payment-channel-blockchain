...
contract TokenNetwork is Utils, Controllable {
    ...
    modifier isSafe() {
        require(
            safety_deprecation_switch == false,
            "TN: network is deprecated"
        );
        _;
    }
    ...
    modifier isOpen(uint256 channel_identifier) {
        require(
            channels[channel_identifier].state == ChannelState.Opened,
            "TN: channel not open"
        );
        _;
    }
    ...
    constructor(
        ...
        require(
            ...
            "TN: invalid participant limit"
        );
        require(
            _token_network_deposit_limit > 0,
            "TN: invalid network deposit limit"
        );
        require(
            _token_network_deposit_limit >= _channel_participant_deposit_limit,
    ...
    function openChannel(
        address participant1,
        address participant2
    ) public isSafe returns (uint256) {
        bytes32 pair_hash;
        uint256 channel_identifier;

        // Red Eyes release token network limit
        require(
            token.balanceOf(address(this)) < token_network_deposit_limit,
            "TN/open: network deposit limit reached"
        );
        ...
        require(
            ...
            "TN/open: channel exists for participants"
        );
        ...
        emit ChannelOpened(channel_identifier, participant1, participant2);
    ...
    function setTotalDeposit(
        ...
        uint256 total_deposit,
        address partner
    ) public isSafe isOpen(channel_identifier) {
        setTotalDepositFor(
            channel_identifier,
            participant,
            total_deposit,
    ...
    ) external isOpen(channel_identifier) {
    ...
    function setTotalWithdraw2(
        ...
        require(
            withdraw_data.total_withdraw > 0,
            "TN/withdraw: total withdraw is zero"
        );
        require(
            block.timestamp < withdraw_data.withdrawable_until,
            "TN/withdraw: expired"
        );
        ...
        require(
            withdraw_data.participant ==
                recoverAddressFromWithdrawMessage(
                    ...
                    withdraw_data.withdrawable_until,
        ...
        require(
            channel_identifier ==
                getChannelIdentifier(withdraw_data.participant, partner),
            "TN/withdraw: channel id mismatch"
        );

        // Read channel state after validating the function input
        Channel storage channel = channels[channel_identifier];
        Participant storage participant_state = channel.participants[
            withdraw_data.participant
        ];
        ...
        require(
            (withdraw_data.total_withdraw + partner_state.withdrawn_amount) <=
                total_deposit,
        ...
        require(
            current_withdraw <= withdraw_data.total_withdraw,
            "TN/withdraw: underflow"
        );
        ...
        require(
            token.transfer(withdraw_data.participant, current_withdraw),
            "TN/withdraw: transfer failed"
        );
    ...
    function cooperativeSettle(
        ...
        require(
            channel_identifier ==
                getChannelIdentifier(data1.participant, data2.participant),
            "TN/coopSettle: channel id mismatch"
        );
        Channel storage channel = channels[channel_identifier];

        Participant storage participant1_state = channel.participants[
            data1.participant
        ];
        Participant storage participant2_state = channel.participants[
            data2.participant
        ];
        ...
        // than required.
        require(
            (data1.total_withdraw + data2.total_withdraw) == total_deposit,
            "TN/coopSettle: incomplete amounts"
        );
        ...
        // check for safety.
        require(
            data1.total_withdraw <= data1.total_withdraw + data2.total_withdraw,
            "TN/coopSettle: overflow"
        );

        if (data1.total_withdraw > 0) {
            this.setTotalWithdraw2(channel_identifier, data1);
        }
        if (data2.total_withdraw > 0) {
            this.setTotalWithdraw2(channel_identifier, data2);
        }
        removeChannelData(
            channel,
            channel_identifier,
    ...
    function closeChannel(
    ...
    ) public isOpen(channel_identifier) {
        require(
            channel_identifier ==
        ...
        settleable_after[channel_identifier] =
        ...
        require(
            closing_participant == recovered_closing_participant_address,
            "TN/close: invalid closing sig"
        );
    ...
    function updateNonClosingBalanceProof(
        ...
        require(
            balance_hash != bytes32(0x0),
            "TN/update: balance hash is zero"
        );
        ...
        require(
            non_closing_participant == recovered_non_closing_participant,
            "TN/update: invalid non-closing sig"
        );
        ...
        require(
            closing_participant == recovered_closing_participant,
            "TN/update: invalid closing sig"
        );

        Participant storage closing_participant_state = channel.participants[
            closing_participant
        ];
        // Make sure the first signature is from the closing participant
        require(
            closing_participant_state.is_the_closer,
            "TN/update: incorrect signature order"
        );
    ...
    function settleChannel2(
        ...
        require(
            channel_identifier ==
                getChannelIdentifier(participant1, participant2),
        ...
        require(
            channel.state == ChannelState.Closed,
            "TN/settle: channel not closed"
        );

        // Settlement window must be over
        require(
            settleable_after[channel_identifier] < block.timestamp,
            "TN/settle: settlement timeout"
        );
        ...
        // 現在、参加者に転送する必要がある最終的なトークン量と、コントラクトにロックしたままにする必要があるトークン量を計算しています。
        // participant1_transferred_amount = このトランザクションで参加者1が受け取るトークンの量。
        // participant2_transferred_amount = このトランザクションで参加者2が受け取るトークンの量。
        // participant1_locked_amount = 契約に残っているトークンの量で、
        // 参加者1から参加者2への保留中の送金を表す。
        // ローカル変数の数に制限があるため、変数を再利用しています。可読性を高めるために、これはさらにリファクタリングできます。
        (
    ...
    function unlock(
        ...
        // on chain.
        (computed_locksroot, unlocked_amount) = getHashAndUnlockedAmount(locks);
        ...
        require(
            unlock_data.locksroot == computed_locksroot,
            "TN/unlock: locksroot mismatch"
        );
        ...
        if (unlocked_amount > 0) {
            require(
                token.transfer(receiver, unlocked_amount),
                "TN/unlock: unlocked transfer failed"
            );
        }
        ...
        if (returned_tokens > 0) {
            require(
                token.transfer(sender, returned_tokens),
                "TN/unlock: returned transfer failed"
            );
        }
    ...
    function getChannelState(
        ...
        address participant1,
        address participant2
    ) external view returns (ChannelState) {
        bytes32 unlock_key1;
        bytes32 unlock_key2;
        ...
        if (
            state == ChannelState.NonExistent &&
            ...
            unlock_key1 = getUnlockIdentifier(
                channel_identifier,
                participant1,
                participant2
            );
            UnlockData storage unlock_data1 = unlock_identifier_to_unlock_data[
                unlock_key1
            ];

            unlock_key2 = getUnlockIdentifier(
                channel_identifier,
                participant2,
                participant1
            );
            UnlockData storage unlock_data2 = unlock_identifier_to_unlock_data[
                unlock_key2
            ];

            if (
                unlock_data1.locked_amount == 0 &&
                unlock_data2.locked_amount == 0
            ) {
        ...
        returns (uint256, uint256, bool, bytes32, uint256, bytes32, uint256)
    {
        ...
        Participant storage participant_state = channels[channel_identifier]
            .participants[participant];
        unlock_key = getUnlockIdentifier(
            channel_identifier,
            participant,
            partner
        );
    ...
    function getUnlockIdentifier(
        ...
        return
            keccak256(abi.encodePacked(channel_identifier, sender, receiver));
    }
    ...
    function setTotalDepositFor(
        ...
        require(
            ...
            "TN/deposit: channel id mismatch"
        );
        require(total_deposit > 0, "TN/deposit: total_deposit is zero");
        require(
            total_deposit <= channel_participant_deposit_limit,
            "TN/deposit: deposit limit reached"
        );
        ...
        unchecked {
            ...
            require(
                added_deposit <= total_deposit,
                "TN/deposit: deposit underflow"
            );
            ...
            // Red Eyes release token network limit
            require(
                token.balanceOf(address(this)) + added_deposit <=
            ...
            // Overflow check
            require(
                channel_deposit >= participant_state.deposit,
                "TN/deposit: deposit overflow"
            );
        }
        ...
        // Do the transfer
        require(
            token.transferFrom(token_owner, address(this), added_deposit),
            "TN/deposit: transfer failed"
        );
    }
    ...
    function storeUnlockData(
        ...
        // amount of tokens
        // ロック解除する転送がある場合、ロックルートとトークンの合計量を保存する。
        if (settle_input.locked_amount == 0) {
            return;
        }

        bytes32 key = getUnlockIdentifier(
            channel_identifier,
            settle_input.participant,
            receiver
        );
    ...
    ) internal view returns (uint256 total_available_deposit) {
        total_available_deposit = (participant1_state.deposit +
    ...
    function verifyBalanceHashData(
        ...
        // バランスプルーフが提供されていない場合、0の値をハッシュ化すると0以外の値が出力されるため、これを別途チェックする必要があります。
        if (
            participant.balance_hash == 0 &&
    ...
    function getHashAndUnlockedAmount(
        bytes memory locks
    ) internal view returns (bytes32, uint256) {
        uint256 length = locks.length;

        // each lock has this form:
        // (locked_amount || unlockable_until || secrethash) = 3 * 32 bytes
    ...
    function getLockedAmountFromLock(
        bytes memory locks,
        uint256 offset
    ) internal view returns (uint256) {
        uint256 unlockable_until;
        uint256 locked_amount;
        uint256 revealed_at;
        bytes32 secrethash;
        ...
        assembly {
            ...
            unlockable_until := mload(add(locks, offset))
        ...
        // the lock's unlockable_until in order for the hash time lock transfer
        // to be successful.
        revealed_at = secret_registry.getSecretRevealBlockTime(secrethash);
        if (revealed_at == 0 || unlockable_until <= revealed_at) {
            locked_amount = 0;
        }
    ...
    function getMaxPossibleReceivableAmount(
        ...
        // ordered values.
        require(
            participant2_max_transferred >= participant1_max_transferred,
            "TN: transfers not ordered"
        );
        ...
        participant1_net_max_received = (participant2_max_transferred -
